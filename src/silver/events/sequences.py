...
def preprocess_for_sequences(df: pd.DataFrame) -> pd.DataFrame:
    ...
    # CRITICAL: use event_id as tiebreaker, NOT provider_event_id.
    # Carries have provider_event_id = NULL (they're synthesised) but
    # were inserted between their surrounding events by carries.py,
    # so event_id (the silver surrogate PK) preserves correct order.
    # Sorting by provider_event_id with na_position='last' would push
    # all carries to the end of their timestamp group, breaking the
    # chronological chain.
    sort_cols = ['match_id', 'period', 'minute', 'second']
    if 'event_id' in out.columns:
        sort_cols.append('event_id')
    elif 'provider_event_id' in out.columns:
...
def populate_silver_sequences(
    ...
    for batch_start in range(0, len(match_ids), batch_size):
        ...
        query = f"""
            ...
            FROM silver.events
            WHERE match_id = ANY(%s)
            ORDER BY match_id, period, minute, second, event_id
        """
