...
| 210 | Assist |
| 214 | Big Chance |
| 328 | First Time (shot was first-time) |
| 55 | Related event ID (assist link) |

### Shot play pattern qualifiers (mutually exclusive — mapped to `shot_play_pattern` column)

| qualifierId | Name | Column value |
|-------------|------|-------------|
| 22 | Regular play | `regular_play` |
| 23 | Fast break | `fast_break` |
| 24 | Set piece (crossed FK) | `set_piece` |
| 25 | From corner | `from_corner` |
| 26 | Free kick (direct) | `free_kick` |
| 160 | Throw-in set piece | `throw_in_set_piece` |
| 9 | Penalty (fallback) | `penalty` |

Priority order: Q22 → Q23 → Q24 → Q25 → Q26 → Q160 → Q9. Penalty is checked last because Q9 also appears on fouls (typeId 4). In practice all 6,448 shots in the dataset carry exactly one of these — zero overlaps.
...
xg                    -- expected goals (shots only)
vaep_offensive        -- VAEP offensive value
vaep_defensive        -- VAEP defensive value
vaep_value            -- VAEP total value
spadl_type_id         -- SPADL action type
spadl_result_id       -- SPADL result (success/fail)
spadl_bodypart_id     -- SPADL body part
...
value_assist          -- qualifier value for assist events
shot_play_pattern     -- shots only: regular_play|fast_break|set_piece|from_corner|free_kick|throw_in_set_piece|penalty
first_time            -- shots only: bool, true if shot was first-time (Q328)
