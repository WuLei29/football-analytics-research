...
The granular event stream for each match. One row per event, including synthesised carry events inserted by the processing pipeline. This is the primary table for all analytical work at the event level.
...
| `second` | SMALLINT | Yes | Second within the minute |
| `timestamp` | TIMESTAMPTZ | Yes | Provider's wall-clock timestamp — `NULL` for carries |
| `team_id` | INT (FK) | Yes | Internal FK — resolved from `source_team_id` via `silver.teams` |
| `source_team_id` | VARCHAR(50) | Yes | Provider's `contestantId` — retained for cross-row team comparisons in sequence logic |
| `player_id` | INT (FK) | Yes | Internal FK — resolved from `source_player_id` via `silver.players`. `NULL` for some event types and all carries |
| `source_player_id` | VARCHAR(50) | Yes | Provider's `playerId` |
| `player_name` | VARCHAR(150) | Yes | Player name as supplied by the provider |
| `jersey_number` | SMALLINT | Yes | Shirt number on the day — extracted from the Team Set Up event (typeId 34) |
| `team_name` | VARCHAR(100) | Yes | Denormalised team name — avoids a join for the most common display queries |
| `opposition_team_name` | VARCHAR(100) | Yes | Denormalised opposition team name |
| `h_a` | VARCHAR(5) | Yes | `home` / `away` — used by possession sequence logic for sequence counters |
| `type_id` | SMALLINT | Yes | Raw provider event type code — `NULL` for carries |
| `event_type` | VARCHAR(50) | No | Human label derived from `type_id` — e.g. `Pass` / `Shot` / `Carry` / `Tackle` |
| `outcome` | VARCHAR(10) | Yes | `success` / `failure` — mapped from provider's binary `0`/`1` |
| `x` | FLOAT | Yes | Event origin x-coordinate — real pitch dimensions (0–105) |
| `y` | FLOAT | Yes | Event origin y-coordinate — real pitch dimensions (0–68) |
| `end_x` | FLOAT | Yes | Destination x — for passes, shots, carries |
| `end_y` | FLOAT | Yes | Destination y |
| `blocked_x` | FLOAT | Yes | x-coordinate of block point — shots and clearances |
| `blocked_y` | FLOAT | Yes | y-coordinate of block point |
| `goal_mouth_z` | FLOAT | Yes | Goal mouth z-coordinate — GK and shot placement analysis |
| `goal_mouth_y` | FLOAT | Yes | Goal mouth y-coordinate |
| `start_zone_value_xt` | FLOAT | Yes | xT value of the origin cell — populated for `Pass` and `Carry` events only |
| `end_zone_value_xt` | FLOAT | Yes | xT value of the destination cell |
| `xt` | FLOAT | Yes | `end_zone_value_xt − start_zone_value_xt` — net threat generated by the action |
| `sequence_id` | VARCHAR(20) | Yes | Possession sequence identifier — e.g. `home_001`, `away_042`. `NULL` on initial load; populated in a second pass by `classify_possession_sequences()` |
...
> **Coordinate scale:** all coordinates are stored in real pitch dimensions — X: 0–105 m, Y: 0–68 m. The provider's original 0–100 percentage scale is converted during ingestion.

> **Synthesised carries:** the pipeline infers carry events between consecutive actions where ball movement occurred without a recorded event. These rows have `source_event_id = NULL`, `provider_event_id = NULL`, `raw_data = NULL`, and `outcome = 'success'`. They are fully first-class rows for all analytical purposes.

> **Sequence columns — two-pass loading:** sequence columns are intentionally `NULL` after the initial event load. They are populated in a dedicated second pass by running `classify_possession_sequences()` on each match, then `UPDATE`-ing the rows in place. The gold layer aggregate `sequences` table is built from these columns.

> **Ingestion pattern:** `source_event_id` is the natural deduplication key for real events, but carries have no provider UUID. Idempotency is enforced at the match level — if `silver.events` already contains rows for a given `match_id`, that file is skipped entirely.

> **FK resolution:** `team_id` and `player_id` are resolved inside the Python pipeline via in-memory caches loaded once per run from `silver.teams` and `silver.players`. `source_team_id` and `source_player_id` are retained in the table to support cross-row comparisons (e.g. sequence start/end detection) without requiring a join back to the teams or players tables.
